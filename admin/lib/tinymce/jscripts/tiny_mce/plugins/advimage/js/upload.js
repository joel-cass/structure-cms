window.onload = function() {
	...
	var onUploadSuccess = function (file, server_data, receivedResponse) {
		...
		if (stcResponse.path) {
			document.getElementById("src").value = stcResponse.path;
			ImageDialog.showPreviewImage(stcResponse.path);
		}
		if (stcResponse.name && document.getElementById("alt").value == "") {
			document.getElementById("alt").value = stcResponse.name;
		}
	...
	var swfu = new SWFUpload({ 
		upload_url : "../../../../upload.php?type=image&s=" + getSessionString(), 
